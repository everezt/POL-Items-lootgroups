    ...
    public partial class mainScreen : Form
    {
        ...
        private void updateItemTemplate_Click(object sender, EventArgs e)
        {
            if (lootItemsList.SelectedIndex >= 0)
            {
                try
                {
                    ...
                    if (!Dice.isValidDieString(itemTriesBox.Text))
                    {
        ...
        private void updateRandItemGroup_Click(object sender, EventArgs e)
        {
            if (lootRandFromGroupList.SelectedIndex >= 0)
            {
                try
                {
                    ...
                    if (!Dice.isValidDieString(randItemTriesBox.Text))
                    {
                        throw new Exception("Can't update! Tries is not valid die string!");
                    }
        ...
        private void updateCorpseCut_Click(object sender, EventArgs e)
        {
            if (lootOnCorpseCutList.SelectedIndex >= 0)
            {
                try
                {
                    ...
                    if (!Dice.isValidDieString(cutAmountBox.Text))
                    {
                        throw new Exception("Can't update! Tries is not valid die string!");
                    }
        ...
        private void updateTemplate_Click(object sender, EventArgs e)
        {
            if (templateList.SelectedIndex >= 0)
            {
                ...
                try
                {
                    if(!Dice.isValidDieString(goldBox.Text) && goldBox.Text != "")
                    {
        ...
        private void simulButton_Click(object sender, EventArgs e)
        {
            if (templateList.SelectedIndex >= 0)
            {
                ...
                /*else if (groupsLoaded && itemsLoaded)
                {
                    Simulator.MakeLoot(lootTemplates[templateList.SelectedIndex], itemGroups, fullItems);
                } else if (itemsLoaded)
                {
                    Simulator.MakeLoot(lootTemplates[templateList.SelectedIndex], null, fullItems);
                } else if (groupsLoaded)
                {
                    Simulator.MakeLoot(lootTemplates[templateList.SelectedIndex], itemGroups, null);
                }
                */
                ...
                List<string> warnings = Simulator.getWarnings();

                if (warnings.Count > 0)
                {
                    simList.Items.Add("");
                    simList.Items.Add("warnings");
                    simList.Items.Add("{");
                    foreach(var warning in warnings)
                    {
                        simList.Items.Add("\t" + warning);
                    }
                    simList.Items.Add("}");
                }
            }
        ...
        {
            string output = "// generated by Pol-Items-lootgroups\r\n";
            output += templateList.Items[templateList.SelectedIndex].ToString() + "\r\n";
            output += "{\r\n";
